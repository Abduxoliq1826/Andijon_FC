...
def goal_view(request):
    ...
    if request.method == 'POST':
        ...
        print(cl)
        st = Statics.objects.get(club=cl)
        if pl.club == cl:
            if cl == gm.host:
                pl.goals += 1
                st.score += 1
...
def add_subs(request):
    ...
    if request.method == 'POST':
        ...
        minute = request.POST.get('minute')
        cl = Club.objects.get(id=club)
        pl = Player.objects.get(id=squad)
        print(pl, cl)
        if cl == pl.club:
            gm = Game.objects.get(id=game)
            plo = Player.objects.get(id=line)
            pl.sub_on += 1
            plo.sub_off += 1
            pl.minute += int(minute) - int(90)
            plo.minute += int(minute)
            plo.save()
            pl.save()
            subs = Substitute.objects.create(
                game_id=game,
                club_id=club,
                squad_id=squad,
                line_id=line,
